...
lollipops.datasets <- function(mafInfo, gene) {
  
  for ( dataset in names(mafInfo) ) {
    ...
    if ( nrow(subsetMaf(maf = mafInfo[[dataset]], includeSyn = FALSE, genes = gene, 
      ...
      cat(paste("This section was skipped since MAF for dataset (", names(mafInfo)[dataset], ") does not contain field with amino acid changes details.\n\n", sep=" "))
      }
    } else {
      cat(paste("Gene", gene, "has no mutations reported in detaset", names(mafInfo)[dataset], ".\n\n", sep=" "))
    }
  }
}
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[1])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[2])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[3])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[4])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[5])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[6])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[7])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[8])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[9])
```
...
lollipops.datasets(mafInfo, names(mutRate.top10.datasets)[10])
```
