...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Save the plot as PNG
  png( file = paste(outDir, "/MAF_oncoplot_pathways_", datasets.list[i], ".png", sep = ""), width = 1800, height = oncoplot_height[[i]], units = "px", res = 200, type="cairo" )
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( pathways_status ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, sampleOrder = sampleOrder, pathways = params$pathways, gene_mar = 10)
  }
