...
if ( params$purple != "none" ){
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    purple_df = purple_dfs %>% 
      ...
      # oncoplot doesn't seem to support custom tags like HD or LOH though. Need to convert it into Del. So using only HD for now, but potentially can manually select what do we want to treat as a deletion based on gene role:
      mutate(
      	ampdel = case_when(
          (event == "HD") ~ "Del", 
          (event == "Amp") ~ "Amp",
