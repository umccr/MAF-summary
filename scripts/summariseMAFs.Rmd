...
mut.details.datasets <- function(mafInfo, datasets, genes) {
  ...
  for ( i in 1:length(datasets) ) {
    ...
    if ( nrow(mut.details.genes) != 0 ) {
      ...
      widges.list[[i]] <- DT::datatable( data = mut.details.genes, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets[i])), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
  ...
  if ( length(datasets.noMut) != 0 ) {
    ...
    cat(paste("Note, gene(s)", genes, "have no mutations reported in the following datasets:", paste(datasets.noMut, collapse = ", "), "\n\n\n\n", sep=" "))
  }
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable( data = mafInfo[[i]]@summary, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "none", extensions = 'Buttons', options = list(pageLength = nrow(mafInfo[[i]]@summary), dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable( data = maftools::getSampleSummary(mafInfo[[i]]), caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
if ( !is.null(MAF_samples.silent.df) ) {
  ...
  DT::datatable( data = MAF_samples.silent.df, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong("Samples with no non-synonymours variants detected")), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable(data = maftools::getGeneSummary(mafInfo[[i]]), caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
if ( !is.null(params$genes_list) ) {
  ...
  genes_list <- vector("list", length(mafFiles))
  names(genes_list) <- datasets.list
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    genes_list[[i]] <- goi[ goi %!in% top_genes[[i]] ]
    
    ##### Remove genes absent in MAF from the genes of interest list
    genes_list[[i]] <- genes_list[[i]][ genes_list[[i]] %!in% goi_absent[[i]] ]
  }
}
...
Oncoplot(s) illustrating different types of mutations observed across samples for the **`r top_genes_no[[i]]` most frequently mutated genes** (mutated in at least `r params$genes_min` % of patients). The side- and top bar-plots present the frequency of mutations in these genes and in individual samples, respectively.

```{r maf_oncoplot, comment = NA, fig.width = 9, fig.height = top_genes_no[[i]]/5}
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 12)
  
  } else {
    cat(paste0("Dataset ", datasets.list[i], " has ", nrow(maftools::getSampleSummary(mafInfo[[i]])), " sample with non-synonymous variants. Cannot create oncoplot for ", nrow(maftools::getSampleSummary(mafInfo[[i]])), " sample.\n\n\n"))
  }
}
```
...
```{r maf_oncoplot_goi, comment = NA, fig.width = 9, fig.height = 10}
##### Draw additional oncoplot if a list of genes of intereset was specified by user 
###### Generate separate plot for each dataset
if ( !is.null(params$genes_list) && length(goi) > 1 ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    cat(paste("\n\nThe following genes from provided gene list are also among the top", top_genes_no[[i]], "most frequently mutated genes and are ignored:\n\n", sep=" "))
    
    print(top_genes_goi[[i]])
    
    plot.new()
    par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
    maftools::oncoplot(maf = mafInfo[[i]], genes = genes_list[[i]], fontSize = 12)
...
### Driver genes detection {.tabset .tabset-fade}
...
Table(s) presenting per-dataset oncodriveCLUST results, including *cluster scores*, *p-values*, *false discovery rate (FDR) values*, *number of mutations in clusters*, as well as the *fraction of mutations in clusters*. **Cluster score** of 1 means that a single hotspot hosts all observed variants. The genes are ordered according to p-values.

```{r maf_oncodrive_table}
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( !is.null(oncodrive.res[[datasets.list[i]]]) ) {
    ...
    widges.list[[i]] <- DT::datatable(data = oncodrive.res[[datasets.list[i]]][[1]], caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable(data = titv.info[[datasets.list[i]]]$raw.counts, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable(data = titv.info[[datasets.list[i]]]$TiTv.fractions, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable(data = titv.info[[datasets.list[i]]]$fraction.contribution, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')) ) %>%
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( !is.null(params$genes_list) ){
    ...
    genes.top <- c(genes.top, genes_list[[i]])
  }
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[1]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[1]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[2]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[2]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[3]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[3]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[4]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[4]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[5]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[5]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[6]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[6]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[7]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[7]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[8]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[8]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[9]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[9]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[10]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[10]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[11]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[11]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[12]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[12]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[13]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[13]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[14]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[14]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[15]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[15]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[16]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[16]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[17]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[17]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[18]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[18]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[19]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[19]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[20]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[20]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[21]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[21]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[22]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[22]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[23]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[23]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[24]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[24]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[25]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[25]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[26]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[26]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[27]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[27]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[28]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[28]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[29]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[29]), silent = TRUE)
} else {
...
if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[30]]) != 0 ) {
  try(lollipops.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[30]), silent = TRUE)
} else {
...
```{r details_mut_goi, comment = NA}
##### Provide detiles information for each dataset for user-defined number of top mutated genes

if ( !is.null(params$genes_list) ) {
  
  mut.details.datasets(mafInfo, datasets.list, unlist(genes_list))
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 12)
  }
