...
for ( i in 1:length(mafFiles) ) {
  ...
  maf.pathways <- subsetMaf(maf = mafInfo[[i]], includeSyn = FALSE, genes = pathways_genes, mafObj = TRUE, query = "Variant_Type != 'CNV'")
  
  ##### Save the plot as PNG
  png( file = paste(outDir, "/MAF_oncoplot_pathways_", datasets.list[i], ".png", sep = ""), width = 1800, height = oncoplot_goi_height[[i]]+100*length(pathways_genes)/4, units = "px", res = 200, type="cairo" )
