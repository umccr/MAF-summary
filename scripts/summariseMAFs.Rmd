...
for( i in 1:length(mutRate.top.datasets) ) {
  if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[1]]) != 0 ) {
    output_plot[[i]] <- try(lollipops.datasets(mafInfo = mafInfo, datasets  = datasets.list, gene  = names(mutRate.top.datasets)[i]), silent = TRUE)
  }
}
...
for( i in 1:length(mutRate.top.datasets) ){
  ...
  if ( nrow(gff[HGNC %in% names(mutRate.top.datasets)[1]]) != 0 ) {
    cat(renderTags(output_plot[[i]])$html)
    cat("\n***\n")
  } else {
...
Tables with detailed information, as as provided in corresponding MAF file(s), for the **`r length(mutRate.top.datasets)` most frequently mutated genes** (mutated in at least `r params$genes_min` % of patients) across all datasets, as well as for the **genes of interest** (if specified).
