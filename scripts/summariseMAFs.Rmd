...
##### Create a new workbook
wb <- createWorkbook("MAF_summary_top_genes_details.xlsx")

##### Add worksheets, one for each dataset
for ( i in 1:length(mafFiles) ) {
  
    addWorksheet(wb, substring(datasets.list[i], 0, 31), tabColour = datasets.colour[[1]][i])
    writeData(wb, sheet = i, mafInfo[[i]]@data[ mafInfo[[i]]@data[, Hugo_Symbol] %in% names(mutRate.top10.datasets), ])
}

saveWorkbook(wb, paste(outDir, "MAF_summary_top_genes_details.xlsx", sep="/"), overwrite = TRUE)
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  widges.list[[i]] <- DT::datatable( data = mafInfo[[i]]@data[ mafInfo[[i]]@data[, Hugo_Symbol] %in% names(mutRate.top10.datasets)[1], ], caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "none", options = list(pageLength = nrow(mafInfo[[i]]@summary), sDom  = '<"top">lrt<"bottom">ip') )
}
