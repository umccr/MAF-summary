...
params:
  ...
  genes_min: 4
  genes_list: '../example_genes_to_subset.txt'
  genes_blacklist: '../example_genes_blacklist.txt'
  samples_list: '../example_samples_to_subset.txt'
  samples_blacklist: '../example_samples_to_subset.txt'
...
NOTE: this report summarises **only non-synonymous variants** (see definitions below).
...
`r if ( !is.na(params$samples_blacklist) ) { c("List of samples excluded from the analysis.") }`
...
if ( params$clinical_info != "none" ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    widges.list[[i]] <- DT::datatable( data = clinicalInfo[[i]], caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets.list[i])), filter = "top", extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE) ) %>%
...
`r if ( !is.na(params$genes_blacklist) ) { c("##### Excluded genes") }`

`r if ( !is.na(params$genes_blacklist) ) { c("List of genes excluded from the analysis.") }`
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE)
    dev.off()
...
if ( !is.na(params$genes_list) && length(goi) > 1 ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( length(genes_list_goi[[i]]) > 1 ) {
      ...
      plot.new()
      par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
      maftools::oncoplot(maf = mafInfo[[i]], genes = genes_list_goi[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE)
      dev.off()
...
```{r oncogenic_pathways, comment = NA, message=FALSE, warning=FALSE}
###### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {
  
  cat(paste(datasets.list[i], "\n\n", sep=" "))
  
  try(OncogenicPathways(maf = mafInfo[[i]]), silent = TRUE)
}
```
...
`r if ( params$clinical_info != "none" ) { c("Various groupwise and pairwise comparisions are performed to identify enriched mutations for every category within provided clinical features.") }`
...
for ( i in 1:length(mafFiles) ) {
  ...
  clinical_enrichment = clinicalEnrichment(maf = mafInfo[[i]], clinicalFeature = clinicalFeatures[[i]])
  try(plotEnrichmentResults(enrich_res = clinical_enrichment, pVal = 0.05), silent = TRUE)
  cat("<br/><br/><br/>")
}
```
...
Every cancer, as it progresses leaves a signature characterized by specific pattern of nucleotide substitutions (see paper [Signatures of mutational processes in human cancer](https://www.ncbi.nlm.nih.gov/pubmed/23945592){target="_blank"} by Alexandrov *et al*). Such signatures are extracted by decomposing matrix of nucleotide substitutions, classified into 96 substitution classes based on immediate bases surrounding the mutated base.

```{r mutational_sign, comment = NA, message=FALSE, warning=FALSE}
...
for ( i in 1:length(mafFiles) ) {
  mutSign[[i]] = invisible(trinucleotideMatrix(maf = mafInfo[[i]], prefix = 'chr', add = TRUE, ref_genome = "BSgenome.Hsapiens.UCSC.hg19"))
}
```

### Mutational signatures {.tabset .tabset-fade}

Extracted signatures are compared against known signatures derived from [Alexandrov *et al*](https://www.ncbi.nlm.nih.gov/pubmed/23945592){target="_blank"}, and cosine similarity is calculated to identify best match. The signatures are also compared against known and validated signatures from Sanger (see here for [list of validated signatures](https://cancer.sanger.ac.uk/cosmic/signatures){target="_blank"}). 

```{r signature_analysis, comment = NA, message=FALSE, warning=FALSE}
##### Generate separate plot for each dataset
##### Create a list to store signature analysis results info for individual datasets
...
for ( i in 1:length(mafFiles) ) {
  ...
  mutextractSign[[i]] = invisible(extractSignatures(mat = mutSign[[i]], nTry = 6, plotBestFitRes = FALSE))
  try(plotSignatures(mutextractSign[[i]], title_size = 0.8), silent = TRUE)
  cat("<br/><br/>")
  
  invisible(pheatmap::pheatmap(mat = mutextractSign[[i]]$coSineSimMat, cluster_rows = FALSE, main = "cosine similarity against validated signatures"))
}
```

***

### Signatures enrichment {.tabset .tabset-fade}

Signatures are assigned to samples and enrichment analysis are performed to detect mutations enriched in every signature identified.

```{r signature_enrichment, comment = NA, message=FALSE, warning=FALSE}
##### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {
  cat(paste(datasets.list[i], "\n\n", sep=" "))
  
  sign.enrichment = invisible(signatureEnrichment(maf = mafInfo[[i]], sig_res = mutextractSign[[i]]))
  cat("<br/><br/>")
  
  invisible(plotEnrichmentResults(enrich_res = sign.enrichment, pVal = 0.05))
}
```
...
APOBEC induced mutations are more frequent in solid tumors and are mainly associated with *C>T* transition events occurring in *TCW motif*. APOBEC enrichment scores are estimated using the method described by [Roberts *et al*](https://www.ncbi.nlm.nih.gov/pubmed/23852170){target="_blank"}. Briefly, enrichment of *C>T* mutations occurring within *TCW motif* over all of the *C>T* mutations in a given sample is compared to background *Cytosines* and *TCWs* occurring within 20bp of mutated bases.

Samples in individual dataset(s) were classified into APOBEC enriched and non-APOBEC enriched groups, and the differences in mutational patterns between them were analysed to identify differentially altered genes.
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE)
  }
