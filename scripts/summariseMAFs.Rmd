...
if ( !all(unlist(strsplit(params$nonSyn_list, split=',', fixed=TRUE)) %in% c( "Frame_Shift_Del","Frame_Shift_Ins","Splice_Site","Translation_Start_Site","Nonsense_Mutation","Nonstop_Mutation","In_Frame_Del","In_Frame_Ins","Missense_Mutation" ) ) ) {
  
  ##### Get the default colours first
  vc_cols = c(RColorBrewer::brewer.pal(11, name = "Paired"), RColorBrewer::brewer.pal(11,name = "Spectral")[1:3],'black', 'violet', 'royalblue', '#7b7060')
  vc_cols = grDevices::adjustcolor(col = vc_cols, alpha.f = 1)
  names(vc_cols) = names = c('Nonstop_Mutation','Frame_Shift_Del','IGR','Missense_Mutation','Silent','Nonsense_Mutation',
  ...
  vc_missing <- unlist(strsplit(params$nonSyn_list, split=',', fixed=TRUE))[ unlist(strsplit(params$nonSyn_list, split=',', fixed=TRUE)) %!in% names(vc_cols) ]
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols)
    dev.off()
    ...
    if ( length(goi[[i]]) > 1 ) {
      ...
      if ( length(goi[[i]]) > 1 ) {
        ...
        if ( length(top_genes_goi[[i]]) > 1  ) {
          ...
          plot.new()
          par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
          maftools::oncoplot(maf = mafInfo[[i]], genes = goi[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols)
          dev.off()
...
for ( i in 1:length(mafFiles) ) {
  ...
  cat(paste("\n\n <b>", datasets.list[i], "</b> \n\n", sep=" "))
   ...
   if ( length(genes_list_goi.nonsyn[[i]]) > 1 && length(genes_list_goi.nonsyn[[i]]) < 26 ) {
    somInter_goi[[i]] <- maftools::somaticInteractions(maf = mafInfo[[i]], genes = genes_list_goi.nonsyn[[i]], pvalue = c(0.05, 0.1), returnAll = TRUE, verbose = FALSE)
    cat("<br/><br/>")
    ...
    cat(paste0("<span style=\"color:#ff0000\">NOTE</span>: ", length(genes_list_goi.nonsyn[[i]]), " genes were provided for ", datasets.list[i], " dataset but only the first **25 genes** were analysed.\n\n"))
     ...
     somInter_goi[[i]] <- maftools::somaticInteractions(maf = mafInfo[[i]], genes = genes_list_goi.nonsyn[[i]][c(1:25)], pvalue = c(0.05, 0.1), returnAll = TRUE, verbose = FALSE)
...
for ( i in 1:length(mafFiles) ) {
  ...
  genes <- unique( unlist(c(as.data.frame(somInter[[i]]$pairs)[ somInter[[i]]$pairs$pValue < 0.05, "gene1" ], as.data.frame(somInter[[i]]$pairs)[ somInter[[i]]$pairs$pValue < 0.05, "gene2" ])) )
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( length(genes_list_goi.nonsyn[[i]]) > 1 ) {
    if ( length(genes_list_goi.nonsyn[[i]]) > 25 ) {
      ...
      genes <- unique( unlist(c(as.data.frame(somInter_goi[[i]]$pairs)[ somInter_goi[[i]]$pairs$pValue < 0.05, "gene1" ], as.data.frame(somInter_goi[[i]]$pairs)[ somInter_goi[[i]]$pairs$pValue < 0.05, "gene2" ])) )
      ...
      genes <- unique( unlist(c(as.data.frame(somInter_goi[[i]]$pairs)[ , "gene1" ], as.data.frame(somInter_goi[[i]]$pairs)[ , "gene2" ])) )
      if ( length(genes) > 0 ) {
        maftools::oncostrip(maf = mafInfo[[i]], genes = genes, top = NULL, fontSize = 0.8, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE)
      } else {
        cat(paste0("**None of detected gene pairs have P-value < 0.05**.\n\n\n"))
...
maftools::coOncoplot(m1 = mafInfo[[1]], m2 = mafInfo[[2]], m1Name = datasets.list[1], m2Name = datasets.list[2], genes = genes, removeNonMutated = TRUE, clinicalFeatures1 = clinicalFeatures[[1]], clinicalFeatures2 = clinicalFeatures[[2]], sortByAnnotation1 = TRUE, sortByAnnotation2 = TRUE, titleFontSize = 1.2, colors = vc_cols)
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols)
  }
