...
mut.details.datasets <- function(mafInfo, datasets, genes) {
  ...
  for ( i in 1:length(datasets) ) {
    ...
    if ( nrow(mut.details.genes) != 0 ) {
      ...
      widges.list[[i]] <- DT::datatable( data = mut.details.genes, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(datasets[i])), filter = "top", options = list(pageLength = 5) ) %>%
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(getGeneSummary(mafInfo[[i]])) < as.numeric(params$genes_no)  ) {
    
    top_genes_no[[i]] <- nrow(getGeneSummary(mafInfo[[i]]))
    params$genes_no <- nrow(getGeneSummary(mafInfo[[i]]))
    
  } else {
    top_genes_no[[i]] <- as.numeric(params$genes_no)
  }
}
...
if ( !is.null(params$genes_list) ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    top_genes[[i]] <- maftools::getGeneSummary(mafInfo[[i]])$Hugo_Symbol[1:params$genes_no]
...
#### `r params$genes_no` most frequently mutated genes

Oncoplot(s) illustrating different types of mutations observed across samples for the **`r params$genes_no` most frequently mutated genes**. The side- and top bar-plots present the frequency of mutations in these genes and in individual samples, respectively.

```{r maf_oncoplot, comment = NA, fig.width = 9, fig.height = nrow(maftools::getSampleSummary(mafInfo[[i]]))/5}
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(maftools::getSampleSummary(mafInfo[[i]])) > 1 ) {
...
if ( !is.null(params$genes_list) && length(goi) > 1 ) {
  ...
  if ( nrow(maftools::getSampleSummary(mafInfo[[i]])) > 1 ) {
    ...
    cat(paste("\n\nThe following genes from provided gene list are also among the top", top_genes_no, "most frequently mutated genes and are ignored:\n\n", sep=" "))
...
for ( i in 1:length(mafFiles) ) {
  ...
  par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
  maftools::plotmafSummary(maf = mafInfo[[i]], top = params$genes_no, rmOutlier = TRUE, addStat = 'median', dashboard = TRUE, titvRaw = FALSE)
...
for ( i in 1:length(mafFiles) ) {
  ...
  genes.top <- maftools::getGeneSummary(mafInfo[[i]])[c(1:params$genes_no), Hugo_Symbol]
...
mutRate.top.datasets <- sort(sort(mutRate.top.datasets, decreasing = TRUE)[unique(names(mutRate.top.datasets))], decreasing = TRUE)[c(1:params$genes_no)]
...
Tables with detailed information, as as provided in corresponding MAF file(s), for the **`r params$genes_no` most frequently mutated genes** across all datasets, as well as for the **genes of interest** (if specified).
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[c(1:params$genes_no)])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
Interactive heatmap(s) with colours indicating low (blue) and high (yellow) number of *various mutations types* (columns) detected in corresponding *genes* (rows). Genes are ordered by the number of reported mutations. The *total number of mutations* in individual genes, as well as the *number of samples with mutations* are also presented in the last three columns. Note, for transparency only the **top `r params$genes_no` mutated genes** are presented.
