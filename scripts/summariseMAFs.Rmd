...
if ( params$purple != "none" ){
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    nonSyn_list <- unique(c(nonSyn_list, purple_df$ampdel))
    
    ##### Add clinical information if provided
    if ( !is.na(clinicalInfo[[i]]) ) {
      mafInfo[[i]] <- maftools::read.maf(maf = maf.data, vc_nonSyn = nonSyn_list, removeDuplicatedVariants = params$remove_duplicated_variants, verbose = FALSE, cnTable = purple_df %>% select(Gene = gene, Sample_name = sample, CN = ampdel), clinicalData = clinicalInfo[[i]])
    } else {
      mafInfo[[i]] <- maftools::read.maf(maf = maf.data, vc_nonSyn = nonSyn_list, removeDuplicatedVariants = params$remove_duplicated_variants, verbose = FALSE, cnTable = purple_df %>% select(Gene = gene, Sample_name = sample, CN = ampdel), clinicalData = NULL)
    }
...
if ( params$cnvkit != "none" ){
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    nonSyn_list <- unique(c(nonSyn_list, cnvkit_df$ampdel))
    
    ##### Add clinical information if provided
    if ( !is.na(clinicalInfo[[i]]) ) {
      mafInfo[[i]] <- maftools::read.maf(maf = maf.data, vc_nonSyn = nonSyn_list, removeDuplicatedVariants = params$remove_duplicated_variants, verbose = FALSE, cnTable = cnvkit_df %>% select(Gene = gene, Sample_name = sample, CN = ampdel), clinicalData = clinicalInfo[[i]])
    } else {
      mafInfo[[i]] <- maftools::read.maf(maf = maf.data, vc_nonSyn = nonSyn_list, removeDuplicatedVariants = params$remove_duplicated_variants, verbose = FALSE, cnTable = cnvkit_df %>% select(Gene = gene, Sample_name = sample, CN = ampdel), clinicalData = NULL)
    }
...
if ( !all(nonSyn_list %in% c( "Frame_Shift_Del","Frame_Shift_Ins","Splice_Site","Translation_Start_Site","Nonsense_Mutation","Nonstop_Mutation","In_Frame_Del","In_Frame_Ins","Missense_Mutation" ) ) ) {
  ...
  vc_cols = c(RColorBrewer::brewer.pal(11, name = "Paired"), RColorBrewer::brewer.pal(11,name = "Spectral")[1:3],'black', 'violet', 'violet', 'royalblue', 'royalblue4', 'skyblue', '#7b7060', '#535c68')
  vc_cols = grDevices::adjustcolor(col = vc_cols, alpha.f = 1)
  names(vc_cols) = names = c('Nonstop_Mutation','Frame_Shift_Del','IGR','Missense_Mutation','Silent','Nonsense_Mutation','RNA','Splice_Site','Intron','Frame_Shift_Ins','In_Frame_Del','ITD','In_Frame_Ins','Translation_Start_Site',"Multi_Hit", 'Amp', 'AMP', 'Del', 'HD', 'LOH', 'Complex_Event', 'pathway')
