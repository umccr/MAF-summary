...
if ( !is.null(params$genes_list) ){
  ...
  genes_list <- unique(read.table(params$genes_list, sep="\t", as.is=TRUE, header=FALSE, row.names=NULL)[,1])
}
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(maftools::getSampleSummary(mafInfo[[i]])) > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = params$genes_no, fontSize = 12)
  
    ##### Draw additional oncoplot if a list of genes of intereset was specified by user 
    if ( !is.null(params$genes_list) && length(genes_list) > 1 ) {
      
      cat("Additional genes of interest\n")
      
      plot.new()
      par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
      maftools::oncoplot(maf = mafInfo[[i]], genes = genes_list, fontSize = 12)
    }
  } else {
...
Tables with detailed information, as as provided in corresponding MAF file(s), for the **`r params$genes_no` most frequently mutated genes** across all datasets.

```{r details_mut_table}
##### Provide detiles information for each dataset for user-defined top mutated genes
##### Create a new workbook
...
##### Provide detiles information for each dataset for user-defined list of genes of interest

if ( !is.null(params$genes_list) ) {
  
  mut.details.datasets(mafInfo, datasets.list, genes_list)
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(maftools::getSampleSummary(mafInfo[[i]])) > 1 ) {
    ...
    plot.new()
    par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = params$genes_no, fontSize = 12)
  }
