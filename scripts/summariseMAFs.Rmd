...
##### Assign different colour to individual cohort
cohorts.colour <- getDatasetsColours(cohorts.list)

##### Create a new workbook
wb <- createWorkbook("MAF_summary.xlsx")

##### Add worksheets, one for each cohort
for ( i in 1:length(mafFiles) ) {
  
    addWorksheet(wb, cohorts.list[i], tabColour = cohorts.colour[[1]][i])
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable( data = mafInfo[[i]]@summary, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;','Overall summary: ', htmltools::strong(cohorts.list[i])), filter = "none", options = list(pageLength = 20, sDom  = '<"top">lrt<"bottom">ip'), height = 100 )
}
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable( data = maftools::getSampleSummary(mafInfo[[i]]), caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', 'Samples summary: ', htmltools::strong(cohorts.list[i])), filter = "top", options = list(pageLength = 5), height = 100 )
}
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable(data = maftools::getGeneSummary(mafInfo[[i]]), caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', 'Genes summary: ', htmltools::strong(cohorts.list[i])), filter = "top", options = list(pageLength = 5), height = 100 )
}
...
###### Generate separate plot for each cohort
###### Create empty list for TiTv info from each cohort
titv.info <- list()
...
for ( i in 1:length(mafFiles) ) {
  ...
  titv.info[[cohorts.list[i]]] <- maftools::titv(maf = mafInfo[[i]], plot = FALSE, useSyn = TRUE)

  maftools::plotTiTv(res = titv.info[[cohorts.list[i]]])
...
##### Add worksheets, three for each cohort
for ( i in 1:length(mafFiles) ) {
  
    addWorksheet(wb, substring(paste0(cohorts.list[i], " (fraction)"), 0, 31), tabColour = cohorts.colour[[1]][i])
    writeData(wb, sheet = i*3-2, titv.info[[cohorts.list[i]]]$fraction.contribution)
    
    addWorksheet(wb, substring(paste0(cohorts.list[i], " (count)"), 0, 31), tabColour = cohorts.colour[[1]][i])
    writeData(wb, sheet = i*3-1, titv.info[[cohorts.list[i]]]$raw.counts)
    
    addWorksheet(wb, substring(paste0(cohorts.list[i], " (TiTv fractions)"), 0, 31), tabColour = cohorts.colour[[1]][i])
    writeData(wb, sheet = i*3, titv.info[[cohorts.list[i]]]$TiTv.fractions)
}
