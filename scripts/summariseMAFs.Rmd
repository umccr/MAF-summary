...
if ( !all(nonSyn_list %in% c( "Frame_Shift_Del","Frame_Shift_Ins","Splice_Site","Translation_Start_Site","Nonsense_Mutation","Nonstop_Mutation","In_Frame_Del","In_Frame_Ins","Missense_Mutation" ) ) ) {
  ...
  vc_cols = c(RColorBrewer::brewer.pal(11, name = "Paired"), RColorBrewer::brewer.pal(11,name = "Spectral")[1:3],'black', 'violet', 'violet', 'royalblue', 'royalblue4', 'skyblue', 'yellowgreen', 'darkmagenta', '#7b7060', '#535c68')
  vc_cols = grDevices::adjustcolor(col = vc_cols, alpha.f = 1)
  names(vc_cols) = names = c('Nonstop_Mutation','Frame_Shift_Del','IGR','Missense_Mutation','Silent','Nonsense_Mutation','RNA','Splice_Site','Intron','Frame_Shift_Ins','In_Frame_Del','ITD','In_Frame_Ins','Translation_Start_Site',"Multi_Hit", 'Amp', 'AMP', 'Del', 'HD', 'LOH', 'Fusion', 'SV', 'Complex_Event', 'pathway')
...
##### Set height for oncoplots
if ( length(pathways_genes) > 100 ) {
  oncoplot_pahtways_height <- 100*length(pathways_genes)/2+100*length(pathways_genes)/6.5
} else if  ( length(pathways_genes) > 19 ) {
  oncoplot_pahtways_height <- 100*length(pathways_genes)/1.5+100*length(pathways_genes)/5
} else {
  oncoplot_pahtways_height <- 900
}
...
for ( i in 1:length(mafFiles) ) {
  ...
  maf.pathways <- subsetMaf(maf = mafInfo[[i]], includeSyn = FALSE, genes = pathways_genes, mafObj = TRUE)
  
  ##### Save the plot as PNG
  png( file = paste(outDir, "/MAF_oncoplot_pathways_", datasets.list[i], ".png", sep = ""), width = 1800, height = oncoplot_pahtways_height, units = "px", res = 200, type="cairo" )
  
  ##### Drawing oncoplots for the top mutated genes in each dataset
  plot.new()
   par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
  maftools::oncoplot(maf = maf.pathways, fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, barcode_mar = 12, sampleOrder = sampleOrder, pathways = params$pathways, gene_mar = 10)
  dev.off()
  ...
  ##### Write gene summary into a file
  ##### Create a new workbook
  wb <- createWorkbook(paste("MAF_oncoplot_pathways_", datasets.list[i], ".xlsx", sep = ""))
  ...
  saveWorkbook(wb, paste(outDir, "/MAF_oncoplot_pathways_", datasets.list[i], ".xlsx", sep = ""), overwrite = TRUE)
}
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( pathways_status ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, sampleOrder = sampleOrder, pathways = params$pathways)
  }
