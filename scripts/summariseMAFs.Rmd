...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show)
    dev.off()
    ...
    if ( length(goi[[i]]) > 1 ) {
      ...
      if ( length(goi[[i]]) > 1 ) {
        ...
        if ( length(top_genes_goi[[i]]) > 1  ) {
          ...
          plot.new()
          par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
          maftools::oncoplot(maf = mafInfo[[i]], genes = goi[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show)
          dev.off()
...
for ( i in 1:length(gisticFiles) ) {
  if ( gisticInfo[[i]]$status ) {
    ...
    plot.new()
    par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
    maftools::gisticOncoPlot(gistic = gisticInfo[[i]]$summary, showTumorSampleBarcodes = params$samples_show)
    dev.off()
...
maftools::coOncoplot(m1 = mafInfo[[1]], m2 = mafInfo[[2]], m1Name = datasets.list[1], m2Name = datasets.list[2], genes = genes, removeNonMutated = TRUE, clinicalFeatures1 = clinicalFeatures[[1]], clinicalFeatures2 = clinicalFeatures[[2]], sortByAnnotation1 = TRUE, sortByAnnotation2 = TRUE, titleFontSize = 1.2, colors = vc_cols, showTumorSampleBarcodes = params$samples_show)
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show)
  }
