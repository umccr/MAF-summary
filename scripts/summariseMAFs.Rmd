...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.7, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, barcode_mar = 5, sampleOrder = sampleOrder)
    dev.off()
  ...
  if ( length(goi[[i]]) > 1 ) {
    ...
    if ( length(goi[[i]]) > 1 ) {
      ...
      if ( length(top_genes_goi[[i]]) > 1  ) {
        ...
        plot.new()
        par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
        maftools::oncoplot(maf = mafInfo[[i]], genes = goi[[i]], fontSize = 0.7, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, barcode_mar = 5, keepGeneOrder = params$genes_keep_order, sampleOrder = sampleOrder)
        dev.off()
...
for ( i in 1:length(mafFiles) ) {
  ...
  plot.new()
  par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
  maftools::oncoplot(maf = maf.pathways, fontSize = 0.7, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, barcode_mar = 5, sampleOrder = sampleOrder, pathways = params$pathways, gene_mar = 10)
  dev.off()
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( length(genes) > 0 ) {
    maftools::oncostrip(maf = mafInfo[[i]], genes = genes, top = NULL, fontSize = 0.7, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, showTumorSampleBarcodes = params$samples_show, barcode_mar = 5, gene_mar = 6)
  } else {
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( length(genes_list_goi.nonsyn[[i]]) > 1 ) {
    if ( length(genes_list_goi.nonsyn[[i]]) > 25 ) {
      ...
      if ( length(genes) > 0 ) {
        maftools::oncostrip(maf = mafInfo[[i]], genes = genes, top = NULL, fontSize = 0.7, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE)
      } else {
        cat(paste0("**None of detected gene pairs have P-value < 0.05**.\n\n\n"))
      }
      cat("<br/><br/>")
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.7, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, sampleOrder = sampleOrder)
  }
  ...
  if ( pathways_status ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], fontSize = 0.7, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, sampleOrder = sampleOrder, pathways = params$pathways, gene_mar = 10)
  }
