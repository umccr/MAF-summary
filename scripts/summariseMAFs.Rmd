  ...
  for ( dataset in datasets ) {
    ...
    if ( nrow(mut) != 0 ) {
      ...
      if ( any(!is.na(as.data.frame(mut)[ , pchange  ]))  ) {
        ...
        pdf(file=paste0(mutationMapsDir, "/", paste(dataset, gene, sep="_"), ".pdf"))
...
if ( params$genes_list != "none" ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( length(genes_list_goi[[i]]) > 100 ) {
      ...
      oncoplot_goi_height[[i]] <- 100*length(genes_list_goi[[i]])/1.5
    } else {
...
###### Generate separate plot for each dataset
if ( params$genes_list != "none" && length(goi) > 1 ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( length(genes_list_goi[[i]]) > 1 ) {
      ...
      plot.new()
      par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
      maftools::oncoplot(maf = mafInfo[[i]], genes = genes_list_goi[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE)
      dev.off()
...
```{r maf_summary_plot, comment = NA, fig.width = 9, fig.height = 10, results="asis"}
###### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {
...
Enrichment of known oncogenic signaling pathways in [TCGA](https://cancergenome.nih.gov/){target="_blank"} cohorts (see paper [Oncogenic Signaling Pathways in The Cancer Genome Atlas](https://www.ncbi.nlm.nih.gov/pubmed/29625050){target="_blank"}).
...
```{r clinical_enrichment, comment = NA, message=FALSE, warning=FALSE, results="asis"}
##### Generate separate plot for each dataset
...
```{r signature_analysis, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 6, results="asis"}
##### Generate separate plot for each dataset
##### Create a list to store signature analysis results info for individual datasets
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Save the plot as PNG
  png( file = paste(outDir, "/MAF_oncoplot_goi_", datasets.list[i], "COSMIC_signatures_comp.png", sep = ""), width = 1200, height = 1200, units = "px", res = 200, type="cairo" )
  plot.new()
  par(mar=c(2,4,2,0.5), oma=c(1.5,2,2,1))
  pheatmap::pheatmap(mat = mutextractSign[[i]]$coSineSimMat, cluster_rows = FALSE, main = "Cosine similarity against validated signatures")
  dev.off()
  ...
  cat("![](",paste(outDir, "/MAF_oncoplot_goi_", datasets.list[i], "COSMIC_signatures_comp.png", sep = ""),")\n")
  cat("<br/>")
  cat("\n</details>\n")
}
```
...
```{r signature_enrichment, comment = NA, message=FALSE, warning=FALSE, results="asis"}
##### Generate separate plot for each dataset
##### Create a list to store signature analysis results info for individual datasets
...
```{r signature_enrichment_genes, comment = NA, message=FALSE, warning=FALSE, results="asis"}
##### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {
  cat(paste("\n\n <b>", datasets.list[i], "</b>\n\n", sep=" "))
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(sub.tbl[!is.na(APOBEC_Enriched), mean(fraction_APOBEC_mutations), 
    ...
    plotApobecDiff(tnm = mutSign[[i]], maf = mafInfo[[i]], pVal = 0.2)
    cat("<br/><br/><br/>")
  }
}
```
...
Tables with detailed information, as as provided in corresponding MAF file(s), for the **`r sum(unlist(top_genes_no))` most frequently mutated genes** (mutated in at least `r params$genes_min` % of patients) across all datasets, as well as for the **genes of interest** (if specified).
...
##### Provide detiles information for each dataset for user-defined number of top mutated genes
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top.datasets)[c(1:max(unlist(top_genes_no[[i]])))], type = "nonsynonymous")
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
if ( params$genes_list != "none" ) {
  ...
  mut.details.datasets(mafInfo, datasets.list, unlist(genes_list_goi), type = "nonsynonymous")
...
`r if ( runGistic ) { c("## CN alterations {.tabset .tabset-fade}") }`

Information about copy-number (CN) alterations (based on  [*GISTIC*](http://software.broadinstitute.org/cancer/software/genepattern/modules/docs/GISTIC_2.0){target="_blank"} programme) is **`r if ( runGistic ) { c("AVAILABLE") } else { c("NOT AVAILABLE") }`**.
...
`r if ( runGistic ) { c("### Tables {.tabset .tabset-fade}") }`
...
```{r gistic_sample_summary, comment = NA, warning=FALSE, eval = runGistic}
##### Present a sample table in the html report
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
`r if ( runGistic ) { c("#### Genes summary") }`
...
```{r gistic_gene_summary, warning=FALSE, eval = runGistic}
##### Present a gene table in the html report
##### Create a list for htmlwidgets
widges.list <- htmltools::tagList()
...
for ( i in 1:length(mafFiles) ) {
  ...
  layout(width  = 900, height = 600, margin = list(l=150, r=10, b=80, t=50, pad=2), titlefont = list(size=16), xaxis = list(tickfont=list(size=10)), yaxis = list(tickfont=list(size=10)))
