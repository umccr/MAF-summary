...
if ( !is.na(params$gistic) ){
  ...
  all.lesions <- list.files(params$gistic, pattern="all_lesions.conf", all.files=FALSE,
    full.names=TRUE)
  amp.genes <- list.files(params$gistic, pattern="amp_genes.conf", all.files=FALSE,
    full.names=TRUE)
  del.genes <- list.files(params$gistic, pattern="del_genes.conf", all.files=FALSE,
    full.names=TRUE)
  scores.gis <- list.files(params$gistic, pattern="scores.gistic", all.files=FALSE,
    full.names=TRUE)

  ##### Check if GISTIC output files exist
  if ( !is.na(all.lesions[1]) && !is.na(amp.genes[1]) && !is.na(del.genes[1]) && !is.na(scores.gis[1]) ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( samples_id_cols[i] %in% names(mafInfo[[i]]@data) && samples_id_cols[i] != "Tumor_Sample_Barcode" ) {
      ...
      if ( runGistic ) {
        ...
        mafInfo[[i]] <- maftools::read.maf(maf = maf.data, vc_nonSyn = params$nonSyn_list, removeDuplicatedVariants = params$remove_duplicated_variants, gisticAllLesionsFile = all.lesions, gisticAmpGenesFile = amp.genes, gisticDelGenesFile = del.genes, gisticScoresFile = scores.gis, verbose = FALSE)
...
if ( !is.na(params$samples_list) ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( runGistic ) {
      ...
      mafInfo[[i]] <- maftools::read.maf(maf = mafInfo[[i]], vc_nonSyn = params$nonSyn_list, removeDuplicatedVariants = params$remove_duplicated_variants, gisticAllLesionsFile = all.lesions, gisticAmpGenesFile = amp.genes, gisticDelGenesFile = del.genes, gisticScoresFile = scores.gis, verbose = FALSE)
