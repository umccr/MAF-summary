  ...
  for ( dataset in datasets ) {
    ...
    if ( nrow(subsetMaf(maf = mafInfo[[dataset]], includeSyn = FALSE, genes = gene, 
      ...
      if ( "HGVSp_Short" %in% maftools::getFields(mafInfo[[dataset]]) ) {
        ...
        plot(lollipopPlot(maf = mafInfo[[dataset]], gene = gene, AACol = "HGVSp_Short",  labelPos = "all" , showMutationRate = TRUE, cBioPortal = TRUE))
        
      } else if ( "aa_mutation" %in% maftools::getFields(mafInfo[[dataset]]) ) {
      
        plot(lollipopPlot(maf = mafInfo[[dataset]], gene = gene, AACol = "aa_mutation",  labelPos = "all" , showMutationRate = TRUE, cBioPortal = TRUE))
...
mut.details.datasets <- function(mafInfo, datasets, gene) {
  ...
  for ( i in 1:length(datasets) ) {
    ...
    if ( nrow(mut.details.gene) != 0 ) {
      ...
      widges.list[[i]] <- DT::datatable( data = mut.details.gene, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;'), filter = "top", options = list(pageLength = 5) )
      
    } else {
      cat(paste("Gene", gene, "has no mutations reported in dataset", datasets[i], "\n\n\n\n", sep=" "))
    }
  }
...
***
NOTE: this report summarised only non-synonymous variants with high/moderate variant consequences, including *frame shift deletions*, *frame shift deletions*, *splice site mutations*, *translation start site mutations* ,*nonsense mutation*, *nonstop mutations*, *in-frame deletion*, *in-frame insertions* and *missense mutation*, and ignores silent variants.
...
for ( i in 1:length(mafFiles) ) {
  ...
  plot.new()
  par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
  maftools::oncoplot(maf = mafInfo[[i]], colors = col, top = 10, fontSize = 12)
}
```
...
for ( i in 1:length(mafFiles) ) {
  ...
  mutRate.top10 <- round(getGeneSummary(x = mafInfo[[i]])[ Hugo_Symbol %in% genes.top10, MutatedSamples]/sampleSize * 100, digits = 2)
  names(mutRate.top10) <- genes.top10
  
  mutRate.top10.datasets <- c( mutRate.top10.datasets, mutRate.top10)
}

mutRate.top10.datasets <- sort(sort(mutRate.top10.datasets, decreasing = TRUE)[unique(names(mutRate.top10.datasets))], decreasing = TRUE)[c(1:10)]
```

### `r names(mutRate.top10.datasets)[1]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[1])
```

### `r names(mutRate.top10.datasets)[2]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[2])
```

### `r names(mutRate.top10.datasets)[3]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[3])
```

### `r names(mutRate.top10.datasets)[4]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[4])
```

### `r names(mutRate.top10.datasets)[5]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[5])
```

### `r names(mutRate.top10.datasets)[6]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[6])
```

### `r names(mutRate.top10.datasets)[7]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[7])
```

### `r names(mutRate.top10.datasets)[8]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[8])
```

### `r names(mutRate.top10.datasets)[9]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[9])
```

### `r names(mutRate.top10.datasets)[10]`
...
lollipops.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[10])
```

## Mutation details {.tabset .tabset-fade}

Tables with detailed information, as as provided in corresponding MAF file(s), for the **10 genes with the highest mutation rate** across all datasets.

```{r details_mut_table}
##### Provide detiles information for each dataset for 1st top mutated gene
##### Create a new workbook
wb <- createWorkbook("MAF_summary_top_mutations_details.xlsx")

##### Add worksheets, one for each dataset
for ( i in 1:length(mafFiles) ) {
  
  ##### Extract mutation details
  mut.details <- mafInfo[[i]]@data[ mafInfo[[i]]@data[, Hugo_Symbol] %in% names(mutRate.top10.datasets), ]

  addWorksheet(wb, substring(datasets.list[i], 0, 31), tabColour = datasets.colour[[1]][i])
  writeData(wb, sheet = i, mut.details[ order(mut.details$Hugo_Symbol) , ])
}

saveWorkbook(wb, paste(outDir, "MAF_summary_top_mutations_details.xlsx", sep="/"), overwrite = TRUE)
```
...
##### Provide detiles information for each dataset for 1st top mutated gene
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[1])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[2])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[3])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[4])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[5])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[6])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[7])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[8])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[9])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
mut.details.datasets(mafInfo, datasets.list, names(mutRate.top10.datasets)[10])
##### Add extra lines to make sure that this section doesn't overlap with the next one
cat("\n\n\n")
```
...
```{r compare_mafs, message = FALSE, warning=FALSE, comment = NA, fig.width = 6, fig.height = 12}
...
```{r mutation_maps_pdf, comment = NA, message=FALSE, warning=FALSE}
###### Save plots into PDF files
###### Generate separate file with plots for each dataset
for ( i in 1:length(mafFiles) ) {
  
  pdf( file = paste(outDir, "/MAF_summary_", datasets.list[i], "_mutation_maps.pdf", sep = ""), height = 4 )

  ###### Generate lollipop plot for 10 most frequently mutated genes across all datasets
  for ( gene in names(mutRate.top10.datasets) ) {

    lollipops.datasets(mafInfo, datasets.list[i], gene)
  }
  dev.off()
}
