...
if ( params$clinical_info != "none" ) {
  for ( i in 1:length(mafFiles) ) {
    ...
    clinical_enrichment = try(clinicalEnrichment(maf = mafInfo[[i]], clinicalFeature = clinicalFeatures[[i]]), silent = TRUE)
...
for ( i in 1:length(mafFiles) ) {
  ...
  plotEnrichmentResults(enrich_res = sign.enrichment[[i]], pVal = 0.05)
  cat("<br/><br/>")
}
```
