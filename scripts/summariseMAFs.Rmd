...
lollipops.datasets <- function(mafInfo, datasets, gene) {
  ...
  for ( dataset in datasets ) {
    ...
    mut = subsetMaf(maf = mafInfo[[dataset]], includeSyn = TRUE, genes = gene, mafObj = FALSE, query = "Variant_Type != 'CNV'")
...
if ( params$samples_list != "none" ) {
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( gisticInfo[[i]]$status ) {
      mafInfo[[i]] <- maftools::read.maf(maf = mafInfo[[i]], vc_nonSyn = unlist(strsplit(params$nonSyn_list, split=',', fixed=TRUE)), removeDuplicatedVariants = params$remove_duplicated_variants, gisticAllLesionsFile = gisticInfo[[i]]$all.lesions, gisticAmpGenesFile = gisticInfo[[i]]$amp.genes, gisticDelGenesFile = gisticInfo[[i]]$del.genes, gisticScoresFile = gisticInfo[[i]]$scores.gis, verbose = FALSE)
...
if ( !is.null(MAF_samples.silent.df) ) {
  DT::datatable( data = MAF_samples.silent.df, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong("Samples with no non-synonymours variants detected")), filter = "top", extensions = 'Buttons', options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy')), width = 800,  escape = FALSE ) %>%
...
if ( params$genes_list != "none" ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    genes_list_goi[[i]] <- goi[[i]][ goi[[i]] %!in% top_genes[[i]] ]
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE)
    dev.off()
...
Oncoplot(s) illustrating different types of mutations observed across samples for the **genes of interest** (if specified). The side and top bar-plots present the frequency of mutations in these genes and in individual samples, respectively. <span style="color:#ff0000">NOTE</span>, the top column bar illustrates the total number of alterations detected only in the genes of interest. 
  ...
  for ( i in 1:length(mafFiles) ) {
    if ( length(goi[[i]]) > 1 ) {
      ...
      if ( length(goi[[i]]) > 1 ) {
        ...
        if ( length(goi[[i]][ goi[[i]] %in% mafInfo[[i]]@data$Hugo_Symbol ]) > 1  ) {
          ...
          plot.new()
          par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
          maftools::oncoplot(maf = mafInfo[[i]], genes = goi[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE)
          dev.off()
        ...
        } else if ( length(goi[[i]][ goi[[i]] %in% mafInfo[[i]]@maf.silent$Hugo_Symbol ]) > 1  ) {
          cat(paste0("**None of the genes of interest have non-synonymous variants detected and for *", paste(goi[[i]][ goi[[i]] %in% mafInfo[[i]]@maf.silent$Hugo_Symbol ], collapse = ", "), "* only silent variants were detected in ", datasets.list[i], " dataset**.\n\n\n"))
...
for ( i in 1:length(mafFiles) ) {
  ...
  tcgaCompare.res <- tcgaCompare(maf = mafInfo[[i]], cohortName = datasets.list[i], primarySite=TRUE)
  cat("<br/><br/>")
}
```
...
```{r oncogenic_pathways, comment = NA, message=FALSE, warning=FALSE, results="asis"}
###### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {
  
  cat(paste("\n\n <b>", datasets.list[i], "</b>\n\n", sep=" "))

  capture.output(OncogenicPathways(maf = mafInfo[[i]]))
  cat("<br/><br/>")
}
```
...
if ( params$clinical_info != "none" ) {
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( any(clinical.enrichment[[i]]$groupwise_comparision$p_value < params$clinical_enrichment_p) ) {
...
Word cloud plot for **most frequently mutated genes** (mutated in at least `r params$genes_min` % of patients). *Size* of each gene is proportional to the total *number of samples* in which it is altered.

```{r gene_cloud, comment = NA, message=FALSE, warning=FALSE, results="asis"}
###### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {
  ...
  par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
  maftools::geneCloud(input = mafInfo[[i]], minMut = 3, top = top_genes_no[[i]])
...
for ( i in 1:length(mafFiles) ) {
  mutSign[[i]] = trinucleotideMatrix(maf = mafInfo[[i]], prefix = 'chr', add = TRUE, ref_genome = "BSgenome.Hsapiens.UCSC.hg19")
}
```
...
Mutational signatures detected in individual dataset(s).
...
for ( i in 1:length(mafFiles) ) {
  ...
  invisible(capture.output(mutextractSign[[i]] <- extractSignatures(mat = mutSign[[i]], nTry = 6, plotBestFitRes = FALSE, pConstant = 0.1)))
  invisible(dev.off())
  
  ##### Plot detected signatures
  png( file = paste(outDir, "/MAF_mutational signatures_", datasets.list[i], ".png", sep = ""), width = 1200, height = nrow(mutextractSign[[i]]$contributions)*300, units = "px", res = 200, type="cairo" )
  try(plotSignatures(mutextractSign[[i]], title_size = 0.8), silent = TRUE)
  invisible(dev.off())
  
  cat("![](",paste(outDir, "/MAF_mutational signatures_", datasets.list[i], ".png", sep = ""),")\n")
  
  ##### Drawing heatmap comparison to all COSMIC signatures
  ##### Save the plot as PNG
  par(mar=c(2,4,2,0.5), oma=c(1.5,2,2,1))
  invisible(capture.output(pheatmap::pheatmap(mat = mutextractSign[[i]]$coSineSimMat, cluster_rows = FALSE, main = "Cosine similarity against validated signatures", cellheight = 10, filename = paste(outDir, "/MAF_COSMIC_signatures_comp_", datasets.list[i], ".png", sep = ""))))
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(sub.tbl[!is.na(APOBEC_Enriched), mean(fraction_APOBEC_mutations), 
    ...
    try(plotApobecDiff(tnm = mutSign[[i]], maf = mafInfo[[i]], pVal = 0.2), silent = TRUE)
    cat("<br/><br/><br/>")
  }
}
```
...
Many disease causing genes in cancer are co-occurring or show strong exclusiveness in their mutation pattern. These **mutually exclusive** or **co-occurring set of genes** were detected using *pair-wise Fisher’s Exact test*. NOTE, only top 25 most frequently mutated genes were considered in the analysis.

### Pair-wise plot

Heatmap of observed pairwise mutation patterns. <span style="color:#48d47d">Green</span> colours denote preferential co-mutation, while <span style="color:#b3408b">pink</span> colours indicate mutual exclusivity. 
...
for ( i in 1:length(mafFiles) ) {
  ...
  somInter[[i]] <- maftools::somaticInteractions(maf = mafInfo[[i]], top = 25, pvalue = c(0.05, 0.1), returnAll = TRUE, verbose = FALSE)
...
for ( i in 1:length(mafFiles) ) {
  ...
  somInter.table <- as.data.frame(somInter[[i]]$pairs)[, colnames(somInter[[i]]$pairs) %!in% c("00","11","01","10") ]
...
for ( i in 1:length(mafFiles) ) {
  ...
  genes <- unique( unlist(as.data.frame(somInter[[i]]$pairs)[ somInter[[i]]$pairs$pValue < 0.05, "gene1" ], as.data.frame(somInter[[i]]$pairs)[ somInter[[i]]$pairs$pValue < 0.05, "gene1" ]) )
  
  maftools::oncostrip(maf = mafInfo[[i]], genes = genes, fontSize = 0.8, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE)
...
for ( i in 1:length(gisticFiles) ) {
  ...
  if ( gisticInfo[[i]]$status ) {
    ...
    ##### Save the genome plot as PDF for each dataset
    maftools::gisticChromPlot(gistic = gisticInfo[[i]]$summary, markBands = "all", file = paste(outDir, "/MAF_gisticGenomePlot_", datasets.list[i], sep = ""), width = 12, height = 5)
    
    ##### Export pdf to png
    gisticChromPlot.image <- image_read_pdf(paste(outDir, "/MAF_gisticGenomePlot_", datasets.list[i], ".pdf", sep = ""), pages = NULL, density = 300)
    image_write(gisticChromPlot.image, path = paste(outDir, "/MAF_gisticGenomePlot_", datasets.list[i], ".png", sep = ""), format = "png")

    file.remove(paste(outDir, "/MAF_gisticGenomePlot_", datasets.list[i], ".pdf", sep = ""))
...
for ( i in 1:length(gisticFiles) ) {
  ...
  if ( gisticInfo[[i]]$status ) {
    ...
    ##### Save the bubble plot as PDF for each dataset
    maftools::gisticBubblePlot(gistic = gisticInfo[[i]]$summary, file = paste(outDir, "/MAF_gisticBubblePlot_", datasets.list[i], sep = ""), width = 8, height = 5)

    ##### Export pdf to png
    gisticBubblePlot.image <- image_read_pdf(paste(outDir, "/MAF_gisticBubblePlot_", datasets.list[i], ".pdf", sep = ""), pages = NULL, density = 300)
    image_write(gisticBubblePlot.image, path = paste(outDir, "/MAF_gisticBubblePlot_", datasets.list[i], ".png", sep = ""), format = "png")

    file.remove(paste(outDir, "/MAF_gisticBubblePlot_", datasets.list[i], ".pdf", sep = ""))
...
for ( i in 1:length(gisticFiles) ) {
  ...
  if ( gisticInfo[[i]]$status ) {
    ...
    plot.new()
    par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
    maftools::gisticOncoPlot(gistic = gisticInfo[[i]]$summary)
    dev.off()
...
maftools::coOncoplot(m1 = mafInfo[[1]], m2 = mafInfo[[2]], m1Name = datasets.list[1], m2Name = datasets.list[2], genes = genes, removeNonMutated = TRUE, clinicalFeatures1 = clinicalFeatures[[1]], clinicalFeatures2 = clinicalFeatures[[2]], sortByAnnotation1 = TRUE, sortByAnnotation2 = TRUE, titleFontSize = 1.2)
```
...
mutRate.top.datasets <- sort(sort(mutRate.top.datasets, decreasing = TRUE)[unique(names(mutRate.top.datasets))], decreasing = TRUE)[c(1:sum(unlist(top_genes_no)))]
```
...
Tables with detailed information, as as provided in corresponding MAF file(s), for the **`r sum(unlist(top_genes_no))` most frequently mutated genes** (mutated in at least `r params$genes_min` % of patients) across all datasets, as well as for the **genes of interest** (if specified) in individual dataset(s).
...
for ( i in 1:length(mafFiles) ) {
  ...
  mut.details <- mafInfo[[i]]@data[ mafInfo[[i]]@data[, Hugo_Symbol] %in% maftools::getGeneSummary(mafInfo[[i]])$Hugo_Symbol[1:top_genes_no[[i]]], ]
...
for ( i in 1:length(mafFiles) ) {
  ...
  saveWidgetFix(as_widget(p), paste0(outDir, "/MAF_sample_summary_heatmap_", datasets.list[i], ".html"), selfcontained = TRUE)
}
...
Interactive heatmap(s) with colours indicating low (blue) and high (yellow) number of *various mutations types* (columns) detected in corresponding *genes* (rows). Genes are ordered by the number of reported mutations. The *total number of mutations* in individual genes, as well as the *number of samples with mutations* are also presented in the last three columns. NOTE, for transparency only the **`r sum(unlist(top_genes_no))` most frequently mutated genes** (mutated in at least `r params$genes_min` % of patients) are presented.
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Save the heatmap as html (PLOTLY)
  saveWidgetFix(as_widget(p), paste0(outDir, "/MAF_gene_summary_heatmap_", datasets.list[i], ".html"), selfcontained = TRUE)
}
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE)
  }
