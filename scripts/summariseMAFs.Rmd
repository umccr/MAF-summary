...
for ( i in 1:length(mafFiles) ) {
  ...
  maf <- subsetMaf(maf = mafInfo[[i]], tsb = tsb, genes = NULL, fields = NULL, query = NULL, mafObj = FALSE, includeSyn = TRUE, dropLevels=TRUE)
  
  ##### Convert the MAF data into Maf object
  mafInfo[[i]] <- maftools::read.maf(maf = maf, vc_nonSyn = unlist(strsplit(params$nonSyn_list, split=',', fixed=TRUE)), removeDuplicatedVariants = params$remove_duplicated_variants, verbose = FALSE, clinicalData = NULL)
...
if ( params$gistic != "none" ){
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    nonSyn_list <- c(unlist(strsplit(params$nonSyn_list, split=',', fixed=TRUE)), "Amp","Del")
...
if ( !all(nonSyn_list %in% c( "Frame_Shift_Del","Frame_Shift_Ins","Splice_Site","Translation_Start_Site","Nonsense_Mutation","Nonstop_Mutation","In_Frame_Del","In_Frame_Ins","Missense_Mutation" ) ) ) {
  ...
  vc_cols = c(RColorBrewer::brewer.pal(11, name = "Paired"), RColorBrewer::brewer.pal(11,name = "Spectral")[1:3],'black', 'violet', 'royalblue', '#7b7060', '#535c68')
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, barcode_mar = 5)
    dev.off()
    ...
    if ( length(goi[[i]]) > 1 ) {
      ...
      if ( length(goi[[i]]) > 1 ) {
        ...
        if ( length(top_genes_goi[[i]]) > 1  ) {
          ...
          plot.new()
          par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
          maftools::oncoplot(maf = mafInfo[[i]], genes = goi[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show, barcode_mar = 5)
          dev.off()
...
for ( i in 1:length(mafFiles) ) {
  ...
  tcgaCompare.res <- tcgaCompare(maf = mafInfo_tcgaCompare[[i]], cohortName = datasets.list[i], primarySite=TRUE)
  } else {
    par(mar=c(1,1,1,1), oma=c(6,2,2,1))
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( length(OncogenicPathways.res[[i]]) > 0 ) {
    ...
    OncogenicPathways.res[[i]] <- as.data.frame(OncogenicPathways.res[[i]])
    
    for( j in 2:nrow(OncogenicPathways.res[[i]]) ){
      
      pathway <- unlist(strsplit(as.character(OncogenicPathways.res[[i]][j,1]), split=' ', fixed=TRUE))
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Prepare table 
  for ( j in 2:length(OncogenicPathways.res[[i]][[1]]) ) {
  ...
  colnames(OncogenicPathways.table) <- c("Pathway", "No of genes", "No of affected genes", "Fraction affected")
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = FALSE, colors = vc_cols, includeColBarCN = FALSE, showTumorSampleBarcodes = params$samples_show)
  }
