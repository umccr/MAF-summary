...
##### Cluster genes
hc <- hclust(as.dist(1-cor(mafCompare.res.common.p, method="pearson")), method="ward.D")

##### Cluster comparisons
hr <- hclust(as.dist(dist(mafCompare.res.common.p, method="euclidean")), method="ward.D")

##### Generate interactive heatmap
p <- heatmaply(mafCompare.res.common.p, Rowv=as.dendrogram(hr), labRow = common_genes, Colv=as.dendrogram(hc), labCol = combNames, viridis(n=256, alpha = 1, begin = 1, end = 0, option = "viridis"), scale="none", dendrogram="row", trace="none", hide_colorbar = FALSE, fontsize_row = 8, label_names=c("Gene","Comparison","P-value")) %>%
