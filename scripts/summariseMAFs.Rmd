...
Mutational signatures presented for individual dataset(s).
...
for ( i in 1:length(mafFiles) ) {
  ...
  invisible(capture.output(mutextractSign[[i]] <- extractSignatures(mat = mutSign[[i]], n = 30, plotBestFitRes = TRUE, pConstant = 0.1)))
  invisible(dev.off())
...
Table(s) with signature contributions (*columns*) in each sample (*rows*) in individual dataset(s).
...
Signatures were assigned to samples and enrichment analysis were performed to detect mutations enriched in individual signatures.
...
Plot(s) illustrating enrichment of individual genes in individual mutational signatures.
