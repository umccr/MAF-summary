...
params:
  maf_dir: '/Users/jmarzec/Documents/GitHub/UMCCR/Cohort-analyses/mutation/projects/KRAS-wt_PAAD/data'
  maf_files: 'KRAS-wt_merged.exonic.maf,KRAS-wt_germline_merged.exonic.maf'
  datasets: 'KRAS-wt-merged-somatic,UMCCR_KRAS-wt-merged-germline'
  genes_min: '4'
  genes_list: '/Users/jmarzec/Documents/GitHub/UMCCR/Cohort-analyses/mutation/projects/KRAS-wt_PAAD/data/KRAS-wt_genes_of_interest.txt'
  out_dir: 'KRAS-wt_merged_somatic_germline_exonic_MAF_report'
output:
  html_document:
    theme: readable
    toc: true
  ...
  for ( dataset in datasets ) {
    ...
    if ( nrow(subsetMaf(maf = mafInfo[[dataset]], includeSyn = FALSE, genes = gene, 
      ...
      if ( "HGVSp_Short" %in% maftools::getFields(mafInfo[[dataset]]) ) {
        ...
        plot.new()
        cat(paste(dataset, "\n\n", sep=" "))
        plot( maftools::lollipopPlot(maf = mafInfo[[dataset]], gene = gene, AACol = "HGVSp_Short",  labelPos = "all" , showMutationRate = TRUE, cBioPortal = TRUE, fn = paste0(mutationMapsDir, "/", paste(dataset, gene, sep="_"))) )
        
      } else if ( "aa_mutation" %in% maftools::getFields(mafInfo[[dataset]]) ) {
        
        plot.new()
        cat(paste(dataset, "\n\n", sep=" "))
        plot( maftools::lollipopPlot(maf = mafInfo[[dataset]], gene = gene, AACol = "aa_mutation",  labelPos = "all" , showMutationRate = TRUE, cBioPortal = TRUE, fn = paste0(mutationMapsDir, "/", paste(dataset, gene, sep="_"))) )
...
```{r maf_oncoplot, comment = NA, fig.width = 9, fig.height = top_genes_no[[i]]/4}
###### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {

  cat(paste(datasets.list[i], "\n\n", sep=" "))
  cat(paste(top_genes_no[[i]], "genes mutated are in at least", params$genes_min, "% of patients\n\n", sep=" "))
...
```{r maf_oncodrive_table, comment=NA, warning=FALSE}
