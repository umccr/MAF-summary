...
if ( params$genes_list != "none" ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    ##### Remove genes absent in MAF from the genes of interest list. Also remove genes with silent mutations
    genes_list_goi[[i]] <- genes_list_goi[[i]][ genes_list_goi[[i]] %!in% goi_absent[[i]] ]
    genes_list_goi[[i]] <- genes_list_goi[[i]][ genes_list_goi[[i]] %!in% genes_list_goi.silent[[i]] ]
...
Word cloud plot for **most frequently mutated genes** (mutated in at least `r gsub(",", ", ", params$genes_min)` % of patients, respectively). *Size* of each gene is proportional to the total *number of samples* in which it is altered.
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Exclusive/co-occurance event analysis on top 25 mutated genes
  somInter_goi[[i]] <- maftools::somaticInteractions(maf = mafInfo[[i]], genes = goi[[i]], pvalue = c(0.05, 0.1), returnAll = TRUE, verbose = FALSE)
  cat("<br/><br/>")
   } else {
     cat(paste0("**Less than 2 genes of interest have non-synonymous variants detected in ", datasets.list[i], " dataset**.\n\n\n"))
   }
...
for ( i in 1:length(mafFiles) ) {
  ...
  somInter.table <- as.data.frame(somInter_goi[[i]]$pairs)[, colnames(somInter_goi[[i]]$pairs) %!in% c("00","11","01","10") ]
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( length(genes_list_goi[[i]]) > 1 ) {
    ...
    genes <- unique( unlist(as.data.frame(somInter_goi[[i]]$pairs)[ somInter_goi[[i]]$pairs$pValue < 0.05, "gene1" ], as.data.frame(somInter_goi[[i]]$pairs)[ somInter_goi[[i]]$pairs$pValue < 0.05, "gene1" ]) )
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( params$genes_list != "none" ){
    genes.top <- unique(c(genes.top, genes_list_goi[[i]]))
  }
