...
Word cloud plot for **most frequently mutated genes** (mutated in at least `r gsub(",", "% and ", params$genes_min)`% of patients, respectively). *Size* of each gene is proportional to the total *number of samples* in which it is altered.
...
for ( i in 1:length(mafFiles) ) {
  mutSign[[i]] = trinucleotideMatrix(maf = mafInfo[[i]], prefix = 'chr', add = TRUE, ref_genome = paste0("BSgenome.Hsapiens.UCSC.hg",params$ucsc_genome_assembly))
}
```
...
Many disease causing genes in cancer are co-occurring or show strong exclusiveness in their mutation pattern. These **mutually exclusive** or **co-occurring set of genes** were detected using *pair-wise Fisher’s Exact test*. <span style=\"color:#ff0000\">NOTE</span>, only top 50 most frequently mutated genes were considered in the analysis.
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( nrow(mafInfo[[i]]@data) > 1 ) {
    somInter[[i]] <- maftools::somaticInteractions(maf = mafInfo[[i]], top = 50, pvalue = c(0.05, 0.1), returnAll = TRUE, verbose = FALSE)
...
for ( i in 1:length(mafFiles) ) {
  ...
  cat(paste("\n\n <b>", datasets.list[i], "</b> \n\n", sep=" "))
   ...
   ##### Exclusive/co-occurance event analysis on top 25 mutated genes
   if ( length(genes_list_goi.nonsyn[[i]]) > 1 && length(genes_list_goi.nonsyn[[i]]) < 51 ) {
    somInter_goi[[i]] <- maftools::somaticInteractions(maf = mafInfo[[i]], genes = goi[[i]], pvalue = c(0.05, 0.1), returnAll = TRUE, verbose = FALSE)
    cat("<br/><br/>")
    
   } else if ( length(genes_list_goi.nonsyn[[i]]) > 50 ) {
     
    cat(paste0("NOTE: ", length(genes_list_goi.nonsyn[[i]]), " genes were provided for ", datasets.list[i], " dataset but only the first **50 genes** were analysed.\n\n"))
     
     somInter_goi[[i]] <- maftools::somaticInteractions(maf = mafInfo[[i]], genes = goi[[i]][c(1:50)], pvalue = c(0.05, 0.1), returnAll = TRUE, verbose = FALSE)
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( length(genes_list_goi.nonsyn[[i]]) > 1 ) {
    if ( length(genes_list_goi.nonsyn[[i]]) > 50 ) {
      cat(paste0("<span style=\"color:#ff0000\">NOTE</span>: ", length(genes_list_goi.nonsyn[[i]]), " genes were provided for ", datasets.list[i], " dataset but only the first **50 genes** were analysed.\n\n"))
    }
