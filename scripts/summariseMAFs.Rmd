...
```{r maf_tcga_cohorts, comment = NA, message=FALSE, warning=FALSE, results="asis"}
###### Generate separate plot for each dataset
for ( i in 1:length(mafFiles) ) {
...
if ( params$clinical_info != "none" ) {
  for ( i in 1:length(mafFiles) ) {
    ...
    clinical.enrichment[[i]] = try(clinicalEnrichment(maf = mafInfo[[i]], clinicalFeature = clinicalFeatures[[i]]), silent = TRUE)
    ...
    if ( any(clinical.enrichment[[i]]$groupwise_comparision$p_value < params$clinical_enrichment_p) ) {
      try(plotEnrichmentResults(enrich_res = clinical.enrichment, pVal = params$clinical_enrichment_p), silent = TRUE)
      cat("<br/><br/>")
      cat("\n***\n")
    } else {
