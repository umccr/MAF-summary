...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    maftools::oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols, includeColBarCN = FALSE)
    dev.off()
    ...
    if ( length(goi[[i]]) > 1 ) {
      ...
      if ( length(goi[[i]]) > 1 ) {
        ...
        if ( length(top_genes_goi[[i]]) > 1  ) {
          ...
          plot.new()
          par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
          maftools::oncoplot(maf = mafInfo[[i]], genes = goi[[i]], fontSize = 0.8, colbar_pathway = TRUE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols, includeColBarCN = FALSE)
          dev.off()
...
for ( i in 1:length(mafFiles) ) {
  ...
  if ( top_genes_no[[i]] > 1 ) {
    ...
    plot.new()
    par(mar=c(1,4,2,0.5), oma=c(1.5,2,2,1))
    oncoplot(maf = mafInfo[[i]], top = top_genes_no[[i]], fontSize = 0.8, colbar_pathway = FALSE, removeNonMutated = FALSE, draw_titv = TRUE, clinicalFeatures = clinicalFeatures[[i]], sortByAnnotation = TRUE, colors = vc_cols, includeColBarCN = FALSE)
  }
