...
This report summarises and visualises mutation data in [Mutation Annotation Format](https://software.broadinstitute.org/software/igv/MutationAnnotationFormat) (MAF) files for the following datasets:
...
Tables with basic information about each dataset, including NCBI build, no. fo samples and genes, no. of different mutation types, as well as the total no. of mutations, as provided in corresponding MAF files.
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable( data = mafInfo[[i]]@summary, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;','Overall summary: ', htmltools::strong(datasets.list[i])), filter = "none", options = list(pageLength = nrow(mafInfo[[i]]@summary), sDom  = '<"top">lrt<"bottom">ip') )
}
...
Tables with samples summary in individual dataset. Each table contains per-sample information (rows) about no. of different types of mutations (columns), as well as the total no. of mutations present in corresponding MAF file.
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable( data = maftools::getSampleSummary(mafInfo[[i]]), caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', 'Samples summary: ', htmltools::strong(datasets.list[i])), filter = "top", options = list(pageLength = 5) )
}
...
Tables with genes summary in individual dataset. Each table contains per-gene information (rows) about no. of different types of mutations (columns), as well as the total no. of mutations present in corresponding MAF file. The last two columns contain the no. of samples with mutations/alterations in the corresponding gene.
...
for ( i in 1:length(mafFiles) ) {
  widges.list[[i]] <- DT::datatable(data = maftools::getGeneSummary(mafInfo[[i]]), caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', 'Genes summary: ', htmltools::strong(datasets.list[i])), filter = "top", options = list(pageLength = 5) )
}
...
for ( i in 1:length(mafFiles) ) {
  ...
  p <- heatmaply(sampleSummary, main = paste0("Samples summary: ", datasets.list[i]), Rowv=NULL, Colv=NULL, scale="none", dendrogram="none", trace="none", hide_colorbar = FALSE, fontsize_row = 8, label_names=c("Sample","Mutation_type","Count")) %>%
...
for ( i in 1:length(mafFiles) ) {
  ...
  ##### Generate interactive heatmap
  p <- heatmaply(geneSummary, main = paste0("Genes summary: ", datasets.list[i]), Rowv=NULL, Colv=NULL, scale="none", dendrogram="none", trace="none", hide_colorbar = FALSE, fontsize_row = 8, label_names=c("Gene","Mutation_type","Count")) %>%
  layout(width  = 900, height = 550, margin = list(l=150, r=10, b=230, t=50, pad=2), titlefont = list(size=16), xaxis = list(tickfont=list(size=10)), yaxis = list(tickfont=list(size=10)))
...
Perform *Fisher Exact* test on all genes for all possible pair-wise comprisons to find differentially mutated genes between queried datasets and to aid identification of global differences in mutation patterns in corresponding *MAFs*. The results are presented in form of a heatmap with rows and columns representing genes and datasets, respectively, and heatmap colours indicating the adjusted *Fisher Exact* test p-values. The colour key is on the left-hand side. Low p-values (yellow) indicate differentially mutated genes between the corresponding datasets. Genes are clustered to facilite the indetification possible differences between individaul datasets.
