...
for ( i in 1:length(mafFiles) ) {
  ...
  plotSignatures(mutextractSign[[i]], title_size = 0.8)
  
  ##### Drawing heatmap comparison to all COSMIC signatures
  ##### Save the plot as PNG
  png( file = paste(outDir, "/MAF_COSMIC_signatures_comp_", datasets.list[i], ".png", sep = ""), width = 1000, height = 1000, units = "px", res = 200, type="cairo" )
  par(mar=c(2,4,2,0.5), oma=c(1.5,2,2,1))
  invisible(capture.output(pheatmap.res <- pheatmap::pheatmap(mat = mutextractSign[[i]]$coSineSimMat, cluster_rows = FALSE, main = "Cosine similarity against validated signatures")))
  invisible(dev.off())
