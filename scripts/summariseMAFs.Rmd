...
if ( !is.null(params$genes_list) ) {
  ...
  if ( length(goi) > 10 ) {
    ...
    oncoplot_goi_height <- 100*length(goi)/2
  } else {
    oncoplot_goi_height <- 900
  }
}
...
if ( !is.null(params$genes_list) && length(goi) > 1 ) {
  ...
  for ( i in 1:length(mafFiles) ) {
    ...
    if ( top_genes_no[[i]] > 1 ) {
      ...
      ##### Save the plot as PNG
      png( file = paste(outDir, "/MAF_oncoplot_goi_", datasets.list[i], ".png", sep = ""), width = 1800, height = oncoplot_goi_height, units = "px", res = 200 )
        
      ##### Drawing oncoplots for the top mutated genes in each dataset
      plot.new()
      par(mar=c(4,4,2,0.5), oma=c(1.5,2,2,1))
      maftools::oncoplot(maf = mafInfo[[i]], genes = genes_list[[i]], fontSize = 12, colbar_pathway = TRUE, removeNonMutated = FALSE)
      dev.off()
