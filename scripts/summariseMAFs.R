...
#
#   Command line use example: Rscript summariseMAFs.R --maf_dir /data --maf_filessimple_somatic_mutation.open.PACA-AU.maf,PACA-CA.icgc.simple_somatic_mutation.maf --datasets ICGC-PACA-AU,ICGC-PACA-CA --genes_no 20 --genes_list genes_of_interest.txt --out_dir MAF_summary
#
...
#   genes_list (optional):  Location and name of a file listing genes of interest to be included in additional oncoplot. The genes are expected to be listed in first column
...
if (is.na(opt$maf_dir) || is.na(opt$maf_files) || is.na(opt$datasets) ) {
  ...
  cat("\ncommand example:\n\nRscript summariseMAFs.R --maf_dir /data --maf_filessimple_somatic_mutation.open.PACA-AU.maf,PACA-CA.icgc.simple_somatic_mutation.maf --datasets ICGC-PACA-AU,ICGC-PACA-CA --genes_no 20--genes_list genes_of_interest.txt --out_dir MAF_summary\n\n")
...
rmarkdown::render(input = "summariseMAFs.Rmd", output_dir = paste(opt$maf_dir, opt$out_dir, "Report", sep = "/"), params = list(maf_dir = opt$maf_dir, maf_files = opt$maf_files, datasets = opt$datasets, genes_no = opt$genes_no, genes_list = opt$genes_list, out_dir = opt$out_dir))
