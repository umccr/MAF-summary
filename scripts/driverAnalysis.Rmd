...
Visualise the intersections between cancer driver genes detected by various tools.

```{r driver_genes_summary, comment = NA, message=FALSE, warning=FALSE}
##### Compute the overlap between cancer driver genes detected by various tools
signif_genes.overlap <- vector("list", length(mafFiles))
names(signif_genes.overlap) <- datasets.list
...
##### Get list of significant driver genes detected by individual methods
method <- c("dNdScv", "dNdSloc", "OncodriveClust")
genes_column <- c("gene_name", "gene_name", "Hugo_Symbol")
analysis_method <- data.frame(method, genes_column)

signif_genes.methods <- vector("list", length(method))
names(signif_genes.methods) <- method
signif_genes.methods
...
for ( i in 1:length(mafFiles) ) {
  ...
  signif_genes.all[[i]] <- unique( c(as.character(signif_genes[[i]]$gene_name), as.character(signif_genes_loc[[i]]$gene_name), as.character(oncodrive.res[[datasets.list[i]]][[1]]$Hugo_Symbol)) )
  
  gene.count <- NULL
  
  for ( gene in as.character(signif_genes.all[[i]]) ) {
    
    if ( gene %in% as.character(signif_genes[[i]]$gene_name) ) {
      
      gene.count <- c(gene.count, 1)
    
    } else {
      gene.count <- c(gene.count, 0)
    }
  }
  
  signif_genes.overlap[[i]] <- gene.count
  
  for ( gene in as.character(signif_genes.all[[i]]) ) {
    
    if ( gene %in% as.character(signif_genes_loc[[i]]$gene_name) ) {
  ...
  signif_genes.overlap[[i]] <- rbind(signif_genes.overlap[[i]], gene.count)
  ...
  signif_genes.overlap[[i]] <- as.data.frame(signif_genes.overlap[[i]])
  names(signif_genes.overlap[[i]]) <- as.character(signif_genes.all[[i]])
  rownames(signif_genes.overlap[[i]]) <- c("dNdScv", "dNdSloc", "oncodriveCLUST")

  upset(signif_genes.overlap[[i]], sets = as.character(signif_genes.all[[i]]), sets.bar.color = "#56B4E9",
